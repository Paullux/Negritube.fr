...
<body> <!-- onload="document.getElementById(1).style.backgroundColor = '#D88851';">² -->
    ...
    <div class="presentation">
      ...
      <div class="choixTitreAudio">
        ...
        for ($i = 0; $i < count($result); $i++) {
          echo "<button class='song_title button_song' id='" . $result[$i]['Numero'] . "' onclick='launchNewMusic(\"" . $result[$i]['Numero'] . "\", \"" . $result[$i]['Titre'] . "\", \"" . $result[$i]['Artiste'] . "\", \"" . $result[$i]['Album'] . "\", \"" . $result[$i]['pochette'] . "\", \"" . count($result) . "\");' type='button'>
  ...
  <input type=hidden id=variableAPasser value=<?= $trackNumber; ?>/>
</body>
...
<script type="text/javascript">currentTrack = audio.getAttribute("src").replace(/^.*[\\\/]/, '').split('.').slice(0, -1).join('.');</script>
<?php $trackNumber = '<script type="text/javascript">document.write(currentTrack);</script>'; ?>
<script>
  ...
  audio.addEventListener('ended', (event) => {
    ...
    clearInterval(k);
    if (shuffleBool) {
      <?= $trackNumber = rand(1,count($result)); ?>
    } else {
      if (<?=$trackNumber?> < <?=count($result)?>) {
        <?php $trackNumber += 1?>;
      } else {
        <?php $trackNumber = 1?>;
      }
    ...
    if (autorenewBool) {
      <?php $index = $trackNumber - 1; echo "launchNewMusic(\"" . $result[$index]['Numero'] . "\", \"" . $result[$index]['Titre'] . "\", \"" . $result[$index]['Artiste'] . "\", \"" . $result[$index]['Album'] . "\", \"" . $result[$index]['pochette'] . "\", \"" . count($result) . "\");"; ?>;
    }
  });
</script>
</html>
